...
public interface SalesByRegionRepo extends JpaRepository<SalesByRegionVO, Integer>{
	 ...
	 @Query(value="SELECT op.order_id,ocat.order_category, pb.base_amount, pb.currency, sdf.category AS subscription_category, oc.order_class, rl.rlist AS r_list,c.id as cust_id,c.fname AS first_name,c.lname AS last_name,ad.address_line1 AS first_address,ad.address_line2 AS second_address,ad.address_category,c.department,ad.city,ad.country,ad.zip_code, " + 
	 		"	 oi.valid_from AS start_date,oi.valid_to AS end_date,oi.effective_date AS order_date,pb.net_amount " + 
	 		"	 FROM order_parent op LEFT JOIN customer c ON op.customer_id = c.id LEFT JOIN oc ON oc.oc_id=op.order_class_id" + 
	 		"	LEFT JOIN customer_addresses_mapping cam ON cam.customer_id=c.id " + 
	 		"	LEFT JOIN addresses ad ON ad.address_id=cam.address_id " + 
	 		...
	 		"	LEFT JOIN region_list rl ON rl.pub_id=c.pub_Id where  (:pubId IS NULL OR c.pub_id = :pubId) AND "+
	 		"    (:paymentStartDate IS NULL OR DATE(op.created_at) >= :paymentStartDate) AND " + 
	 		 ...
	 		 + "AND (:region IS NULL OR rl.rlist=:region) GROUP BY op.order_id",countQuery="SELECT op.order_id,ocat.order_category, pb.base_amount, pb.currency, sdf.category AS subscription_category, oc.order_class, rl.rlist AS r_list,c.id as cust_id,c.fname AS first_name,c.lname AS last_name,ad.address_line1 AS first_address,ad.address_line2 AS second_address,ad.address_category,c.department,ad.city,ad.country,ad.zip_code, " + 
	 		 		"	 			 oi.valid_from AS start_date,oi.valid_to AS end_date,oi.effective_date AS order_date,pb.net_amount  " + 
	 		 		"	 			 FROM order_parent op LEFT JOIN customer c ON op.customer_id = c.id LEFT JOIN oc ON oc.oc_id=op.order_class_id " + 
	 		 		"	 			LEFT JOIN customer_addresses_mapping cam ON cam.customer_id=c.id " + 
	 		 		"	 			LEFT JOIN addresses ad ON ad.address_id=cam.address_id " + 
